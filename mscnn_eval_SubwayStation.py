...
# 参数设置
eval_dir = 'eval/subway/'
data_test_im = 'Data_original/Data_im/subway_test_im/'
data_test_index = 'Data_original/dir_name_subway.txt'

FLAGS = tf.app.flags.FLAGS
tf.app.flags.DEFINE_string('eval_dir', eval_dir, """日志目录""")
tf.app.flags.DEFINE_string('data_test_im', data_test_im, """测试集图片""")
tf.app.flags.DEFINE_string('data_subway_test_index', data_test_index, """测试集图片""")

def Init_Data_gt():
    f = open(data_test_index,'w')
    img_save_path  = data_test_im
    img_names = os.listdir(img_save_path)
...
#     loss_op = mscnn.loss(predict_op, labels)
    ...
    dir_file = open(FLAGS.data_subway_test_index)
    ...
    for file_name in dir_names:
        step += 1
        print(file_name)
        im_name, gt_count = file_name.split(' ')
        gt_count = int(gt_count.split('\n')[0])  # 分离出最后一个回车符
        print(gt_count)
...
#         loss_value = sess.run(loss_op, feed_dict={images: batch_xs, labels: batch_ys})
        ...
        print("time: %s\t counting:%.7f\t predict:%.7f\t diff:%.7f" % \
              ((end - start), gt_count, sum(sum(sum(predict[0]))),
               gt_count-sum(sum(sum(predict[0])))))

        sum_ab = abs(gt_count - sum(sum(sum(predict[0]))))
        sum_all_mae += sum_ab
